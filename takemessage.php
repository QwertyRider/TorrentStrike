  ...
  {                                                                                //////  MM  ///
    ...
    $from_is = $_POST['pmees'];
     ...
     $subject = trim($_POST['subject']);
         if (!$subject)
         stderr('Error','Must enter a Subject.');

         $query = "INSERT INTO messages (sender, receiver, added, msg, subject, location, poster) ". "SELECT $sender_id, u.id, '" . get_date_time() . "', " . sqlesc($msg) . ", " . sqlesc($subject) . ", 1, $sender_id " . $from_is; mysql_query($query) or sqlerr(__FILE__, __LINE__);
  ...
  else
  {
      ...
      stderr('Error','Please enter a subject.');
        ...
        //  $location = ($save == 'yes') ? "both" : "in";
          ...
          $res = mysql_query("SELECT acceptpms, notifs, email, UNIX_TIMESTAMP(last_access) as la FROM users WHERE id=$receiver") or sqlerr(__FILE__, __LINE__);
...
EOD;
 ...
 elseif ($arr["saved"] == "yes")
   mysql_query("UPDATE messages SET location = '0' WHERE id=$origmsg") or sqlerr(__FILE__, __LINE__);
             ...
             if (!$returnto)
                     $returnto = "$BASEURL/messages.php?action=viewmailbox";
          }
