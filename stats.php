...
else
{
	...
	{
		print("<tr><td class=rowhead><a href=userdetails.php?id=".$uper['id']."><b>".$uper['name']."</b></a></td>\n");
		print("<td class=rowhead " . ($uper['last']?(">".$uper['last']." (".get_elapsed_time(sql_timestamp_to_unix_timestamp($uper['last']))." ago)"):"align=center>---") . "</td>\n");
		print("<td class=rowhead align=right>" . $uper['n_t'] . "</td>\n");
		print("<td class=rowhead align=right>" . ($n_tor > 0?number_format(100 * $uper['n_t']/$n_tor,1)."%":"---") . "</td>\n");
		print("<td class=rowhead align=right>" . $uper['n_p']."</td>\n");
		print("<td class=rowhead align=right>" . ($n_peers > 0?number_format(100 * $uper['n_p']/$n_peers,1)."%":"---") . "</td></tr>\n");
	}
	end_table();
	end_frame();
}
...
else
{
  ...
  $res = mysql_query("SELECT c.name, MAX(t.added) AS last, COUNT(DISTINCT t.id) AS n_t, COUNT(p.id) AS n_p
	...
	while ($cat = mysql_fetch_array($res))
	{
		...
		print("<td class=rowhead " . ($cat['last']?(">".$cat['last']." (".get_elapsed_time(sql_timestamp_to_unix_timestamp($cat['last']))." ago)"):"align = center>---") ."</td>");
		print("<td class=rowhead align=right>" . $cat['n_t'] . "</td>");
		print("<td class=rowhead align=right>" . number_format(100 * $cat['n_t']/$n_tor,1) . "%</td>");
		print("<td class=rowhead align=right>" . $cat['n_p'] . "</td>");
		print("<td class=rowhead align=right>" . ($n_peers > 0?number_format(100 * $cat['n_p']/$n_peers,1)."%":"---") . "</td>\n");
	}
	end_table();
	end_frame();
}
