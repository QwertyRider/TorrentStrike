...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
		...
		if ( $search_id == 'newposts' || $search_id == 'egosearch' || ( $search_author != '' && $search_keywords == '' )  )
		{
			if ( $search_id == 'newposts' )
			{
				...
				else
				{
					...
					redirect(append_sid(basefile . "page=login&redirect=" . rawurlencode("index.php?option=com_forum&page=search&search_id=newposts"), true));
				}
			...
			else if ( $search_id == 'egosearch' )
			{
				...
				else
				{
					...
					redirect(append_sid(basefile . "page=login&redirect=" . rawurlencode("index.php?option=com_forum&page=search&search_id=egosearch"), true));
				}
	...
	if ( $search_results != '' )
	{
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			$forum_url = append_sid(basefile . "page=viewforum&" . POST_FORUM_URL . '=' . $searchset[$i]['forum_id']);
			//$topic_url = append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . "&amp;highlight=$highlight_active");
			$topic_url = append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . "&amp;highlight=$highlight_active");
			//$post_url = append_sid("viewtopic.$phpEx?" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . "&amp;highlight=$highlight_active") . '#' . $searchset[$i]['post_id'];
			$post_url = append_sid(basefile . "page=viewtopic&" . POST_POST_URL . '=' . $searchset[$i]['post_id'] . "&amp;highlight=$highlight_active") . '#' . $searchset[$i]['post_id'];
			...
			if ( $show_results == 'posts' )
			{
				...
				$poster = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid(basefile . "page=profile&mode=viewprofile&amp;" . POST_USERS_URL . "=" . $searchset[$i]['user_id']) . '">' : '';
			...
			else
			{
				...
				if ( $searchset[$i]['topic_status'] == TOPIC_MOVED )
				{
					...
					$folder_image = '<img src="' . $phpBB2_install_url . $images['folder'] . '" alt="' . $lang['No_new_posts'] . '" />';
					$newest_post_img = '';
				}
				else
				{
				...
				$topic_author = ( $searchset[$i]['user_id'] != ANONYMOUS ) ? '<a href="' . append_sid(basefile . "page=profile&mode=viewprofile&amp;" . POST_USERS_URL . '=' . $searchset[$i]['user_id']) . '">' : '';
				...
				$last_post_author = ( $searchset[$i]['id2'] == ANONYMOUS ) ? ( ($searchset[$i]['post_username2'] != '' ) ? $searchset[$i]['post_username2'] . ' ' : $lang['Guest'] . ' ' ) : '<a href="' . append_sid(basefile . "page=profile&mode=viewprofile&amp;" . POST_USERS_URL . '='  . $searchset[$i]['id2']) . '">' . $searchset[$i]['user2'] . '</a>';
				...
				$last_post_url = '<a href="' . append_sid(basefile . "page=viewtopic&"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '"><img src="' . $phpBB2_install_url . $images['icon_latest_reply'] . '" alt="' . $lang['View_latest_post'] . '" title="' . $lang['View_latest_post'] . '" border="0" /></a>';
		...
		$base_url = basefile . "page=search&search_id=$search_id&search_author=$search_author";
...
$template->assign_vars(array(
	...
	'S_SEARCH_ACTION' => append_sid(basefile . "page=search&mode=results"),
