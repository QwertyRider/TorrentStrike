...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $row['user_viewemail'] || $userdata['user_level'] == ADMIN )
	{
		...
		if ( isset($HTTP_POST_VARS['submit']) )
		{
			...
			if ( !$error )
			{
				...
				if ( $result = $db->sql_query($sql) )
				{
					...
					$message = $lang['Email_sent'] . '<br /><br />' . sprintf($lang['Click_return_index'],  '<a href="' . append_sid(basefile . "") . '">', '</a>');
		...
		$template->assign_vars(array(
			...
			'S_POST_ACTION' => append_sid(basefile . "page=profile&mode=email&amp;" . POST_USERS_URL . "=$user_id"), 
