...
function show_coppa()
{
	...
	$template->assign_vars(array(
		...
		"U_AGREE_OVER13" => append_sid(basefile . "page=profile&mode=register&amp;agreed=true"),
		//"U_AGREE_UNDER13" => append_sid("profile.$phpEx?mode=register&amp;agreed=true&amp;coppa=true"))
		"U_AGREE_UNDER13" => append_sid(basefile . "page=profile&mode=register&amp;agreed=true&amp;coppa=true"))
	);
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	if ( !$error )
	{
		...
		if ( $mode == 'editprofile' )
		{
			...
			/////////////////////////////////////////////////////////
			// TBDev integration : Update TBDev password ,email and style
			/////////////////////////////////////////////////////////
			
			update_TBDevpassword ($username, $password_confirm);
			update_TBDevemail ($username, $email);
			
			$sql = "SELECT style_name FROM " . THEMES_TABLE . " WHERE themes_id = '" . $user_style . "'";
			if (!($result = $db->sql_query ($sql))) message_die(GENERAL_ERROR, 'Could not obtain users style', '', __LINE__, __FILE__, $sql);
			if ($row = $db->sql_fetchrow ($result)) update_TBDevstyle ($username, $row["style_name"]);
			
			// --- TBDev integration end
        ...
        if(substr(basefile, 0, 4) != "http"){
				  $url = $portal_install_url . basefile . 'page=profile&mode=activate&' . POST_USERS_URL . '=' . $user_id . '&act_key=' . $user_actkey;
				}
				else{
				  $url = basefile . 'page=profile&mode=activate&' . POST_USERS_URL . '=' . $user_id . '&act_key=' . $user_actkey;
				}				
				...
				$message = $lang['Profile_updated_inactive'] . '<br /><br />' . sprintf($lang['Click_return_index'],  '<a href="' . append_sid(basefile . "") . '">', '</a>');
			}
			else
			{
				//$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_index'],  '<a href="' . append_sid("index.$phpEx") . '">', '</a>');
				$message = $lang['Profile_updated'] . '<br /><br />' . sprintf($lang['Click_return_index'],  '<a href="' . append_sid(basefile . "") . '">', '</a>');
			}

			$template->assign_vars(array(
				//"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid("index.$phpEx") . '">')
				"META" => '<meta http-equiv="refresh" content="5;url=' . append_sid(basefile . "") . '">')
			);
		...
		else
		{
			...
			$message = $message . '<br /><br />' . sprintf($lang['Click_return_index'],  '<a href="' . append_sid(basefile) . '">', '</a>');
...
else
{
	...
	if ( $user_avatar_type )
	{
		switch( $user_avatar_type )
		{
			case USER_AVATAR_UPLOAD:
				$avatar_img = ( $board_config['allow_avatar_upload'] ) ? '<img src="' . $phpBB2_install_url . $board_config['avatar_path'] . '/' . $user_avatar . '" alt="" />' : '';
				break;
			case USER_AVATAR_REMOTE:
				$avatar_img = ( $board_config['allow_avatar_remote'] ) ? '<img src="' . $user_avatar . '" alt="" />' : '';
				break;
			case USER_AVATAR_GALLERY:
				$avatar_img = ( $board_config['allow_avatar_local'] ) ? '<img src="' . $phpBB2_install_url . $board_config['avatar_gallery_path'] . '/' . $user_avatar . '" alt="" />' : '';
				break;
		}
	}
	...
	if (!empty($board_config['enable_confirm']) && $mode == 'register')
	{
		...
		$confirm_image = (@extension_loaded('zlib')) ? '<img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id") . '" alt="" title="" />' : '<img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=1") . '" alt="" title="" /><img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=2") . '" alt="" title="" /><img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=3") . '" alt="" title="" /><img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=4") . '" alt="" title="" /><img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=5") . '" alt="" title="" /><img src="' . append_sid(basefile . "page=profile&mode=confirm&id=$confirm_id&c=6") . '" alt="" title="" />';
	...
	$template->assign_vars(array(
		...
		'S_PROFILE_ACTION' => append_sid(basefile . "page=profile"))
	);
	...
	if ( $mode != 'register' )
	{
		if ( $userdata['user_allowavatar'] && ( $board_config['allow_avatar_upload'] || $board_config['allow_avatar_local'] || $board_config['allow_avatar_remote'] ) )
		{
			...
			if ( $board_config['allow_avatar_upload'] && file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_path'])) )
			{
				if ( $form_enctype != '' )
				{
			...
			if ( $board_config['allow_avatar_local'] && file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_gallery_path'])) )
			{
