...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_message, $poll_title, &$poll_options, &$poll_length)
{
	...
	$meta = '<meta http-equiv="refresh" content="3;url=' . append_sid(basefile . "page=viewtopic&" . POST_POST_URL . "=" . $post_id) . '#' . $post_id . '">';
	//$message = $lang['Stored'] . '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . "=" . $post_id) . '#' . $post_id . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id") . '">', '</a>');
	$message = $lang['Stored'] . '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid(basefile . "page=viewtopic&" . POST_POST_URL . "=" . $post_id) . '#' . $post_id . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid(basefile . "page=viewforum&" . POST_FORUM_URL . "=$forum_id") . '">', '</a>');
...
function delete_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id)
{
	...
	else
	{
		...
		$meta = '<meta http-equiv="refresh" content="3;url=' . append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . '=' . $topic_id) . '">';
		...
		$message = (($mode == 'poll_delete') ? $lang['Poll_delete'] : $lang['Deleted']) . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
	}

	//$message .=  '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id") . '">', '</a>');
	$message .=  '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid(basefile . "page=viewforum&" . POST_FORUM_URL . "=$forum_id") . '">', '</a>');
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				if (sizeof($bcc_list_ary))
				{
					...
					while (list($user_lang, $bcc_list) = each($bcc_list_ary))
					{
    				  ...
    				  $url = $portal_install_url . basefile . "page=viewtopic&" . POST_POST_URL . "=$post_id#$post_id";
    				  $url2 = $portal_install_url . basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&unwatch=topic";
    				}
    				else{
    				  $url = basefile . "page=viewtopic&" . POST_POST_URL . "=$post_id#$post_id";
    				  $url2 = basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&unwatch=topic";
    				}
...
{
	global $db, $board_config, $template, $lang, $images, $theme, $phpEx, $phpBB2_install_url, $phpbb_root_path;
		...
		if ($num_smilies)
		{
			...
			while (list($smile_url, $data) = @each($rowset))
			{
				...
				$template->assign_block_vars('smilies_row.smilies_col', array(
					...
					'SMILEY_IMG' => $phpBB2_install_url ."/". $board_config['smilies_path'] . '/' . $smile_url,
			...
			if ($mode == 'inline' && $num_smilies > $inline_rows * $inline_columns)
			{
				...
				$template->assign_vars(array(
					...
					'U_MORE_SMILIES' => append_sid(str_replace('index.php', 'index2.php', basefile) . "page=posting&mode=smilies"))
				);
			}
