...
{
	global $board_config, $userdata, $phpbb_root_path;

	$avatar_file = basename($avatar_file);
	if ( $avatar_type == USER_AVATAR_UPLOAD && $avatar_file != '' )
	{
		if ( @file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_path'] . '/' . $avatar_file)) )
		{
			//@unlink('./' . $board_config['avatar_path'] . '/' . $avatar_file);
			@unlink($phpbb_root_path . $board_config['avatar_path'] . '/' . $avatar_file);
		}
	}
...
function user_avatar_gallery($mode, &$error, &$error_msg, $avatar_filename, $avatar_category)
{
	...
	if ( file_exists(@phpbb_realpath($phpbb_root_path . $board_config['avatar_gallery_path'] . '/' . $avatar_category . '/' . $avatar_filename)) && ($mode == 'editprofile') )
	{
...
{
	global $board_config, $db, $lang, $phpbb_root_path;
	...
	if ( $avatar_mode == 'remote' && preg_match('/^(http:\/\/)?([\w\-\.]+)\:?([0-9]*)\/(.*)$/', $avatar_filename, $url_ary) )
	{
		...
		if ( !$error && $avatar_filesize > 0 && $avatar_filesize < $board_config['avatar_filesize'] )
		{
			...
			$tmp_path = ( !@$ini_val('safe_mode') ) ? '/tmp' : $phpbb_root_path . $board_config['avatar_path'] . '/tmp';
	...
	if ( $width > 0 && $height > 0 && $width <= $board_config['avatar_max_width'] && $height <= $board_config['avatar_max_height'] )
	{
		...
		if( $avatar_mode == 'remote' )
		{
			...
			@copy($tmp_filename, $phpbb_root_path . $board_config['avatar_path'] . "/$new_filename");
			@unlink($tmp_filename);
		}
		else
		{
			...
			$move_file($avatar_filename, $phpbb_root_path . $board_config['avatar_path'] . "/$new_filename");
		}

		//@chmod('./' . $board_config['avatar_path'] . "/$new_filename", 0777);
		@chmod($phpbb_root_path . $board_config['avatar_path'] . "/$new_filename", 0777);
...
function display_avatar_gallery($mode, &$category, &$user_id, &$email, &$current_email, &$coppa, &$username, &$email, &$new_password, &$cur_password, &$password_confirm, &$icq, &$aim, &$msn, &$yim, &$website, &$location, &$occupation, &$interests, &$signature, &$viewemail, &$notifypm, &$popup_pm, &$notifyreply, &$attachsig, &$allowhtml, &$allowbbcode, &$allowsmilies, &$hideonline, &$style, &$language, &$timezone, &$dateformat, &$session_id)
{
	...
	global $phpbb_root_path, $phpEx, $phpbb_root_path;
	...
	$template->assign_vars(array(
		...
		'S_PROFILE_ACTION' => append_sid(basefile . "page=profile&mode=$mode"), 
