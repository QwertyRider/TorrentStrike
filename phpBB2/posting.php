...
// FIXME: In Cancel delete post: redirect is unable to parle a full url !
...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $post_id )
	{
		...
		$redirect = relativebasefile . "page=viewtopic&" . POST_POST_URL . "=$post_id";
		$post_append = "#$post_id";
	}
	else if ( $topic_id )
	{
...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
...
if ( !$is_auth[$is_auth_type] )
{
	...
	redirect(append_sid(relativebasefile . "page=login&redirect=" . rawurlencode("" . basefile . "page=posting&" . $redirect), true));
}
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => append_sid(basefile . "page=posting"),
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		$message .=  '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
		return;
	}
	else
...
$template->assign_vars(array(
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? append_sid(basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;postorder=desc") : '', 
	//'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? append_sid("posting.$phpEx?mode=topicreview&amp;" . POST_TOPIC_URL . "=$topic_id") : '', 
	//FIXME:
	'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? append_sid(str_replace("index.php", "index2.php", basefile) . "page=posting&amp;mode=topicreview&amp;" . POST_TOPIC_URL . "=$topic_id") : '', 
	'PHPURL' => $phpBB2_install_url,
	...
	'S_POST_ACTION' => append_sid(basefile . "page=posting"),
