...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $topic_id )
	{
		...
		$redirect = relativebasefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id";
	}
	else if ( $forum_id )
	{
		//$redirect = "viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id";
		$redirect = relativebasefile . "page=viewforum&" . POST_FORUM_URL . "=$forum_id";
	}
	else
	{
		//$redirect = "index.$phpEx";
		$redirect = relativebasefile . "page=index";
	}
...
switch( $mode )
{
	case 'delete':
		...
		if ( $confirm )
		{
  		if ( empty($HTTP_POST_VARS['topic_id_list']) && empty($topic_id) )
			...
			if ( !empty($topic_id) )
			{
				...
				$redirect_page = basefile . "page=viewforum&" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
				$l_redirect = sprintf($lang['Click_return_forum'], '<a href="' . $redirect_page . '">', '</a>');
			}
			else
			{
				//$redirect_page = "modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
				$redirect_page = basefile . "page=modcp&" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_CONFIRM_ACTION' => append_sid(basefile . "page=modcp"),
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				...
				$redirect_page = basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
			...
			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . basefile . "page=viewforum&" . POST_FORUM_URL . "=$old_forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_MODCP_ACTION' => append_sid(basefile . "page=modcp"),
				'S_HIDDEN_FIELDS' => $hidden_fields)
			);
	...
	case 'lock':
		...
		$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . basefile . "page=viewforum&" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				$template->assign_vars(array(
					...
					'META' => '<meta http-equiv="refresh" content="3;url=' . basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">')
				);
				...
				$message = $lang['Topic_split'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . basefile . "page=viewtopic&" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
				return;
			}
		}
		else
		{
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'S_SPLIT_ACTION' => append_sid(basefile . "page=modcp"),
	...
	case 'ip':
		...
		$template->assign_vars(array(
			...
			'U_LOOKUP_IP' => basefile . "page=modcp&mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=$ip_this_post&amp;sid=" . $userdata['session_id'])
		);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('iprow', array(
					...
					'U_LOOKUP_IP' => basefile . "page=modcp&mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=" . $row['poster_ip'] . "&amp;sid=" . $userdata['session_id'])
				);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('userrow', array(
					...
					'U_PROFILE' => ($id == ANONYMOUS) ? basefile . "page=modcp&mode=ip&amp;" . POST_POST_URL . "=" . $post_id . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id'] : append_sid(basefile . "page=profile&mode=viewprofile&amp;" . POST_USERS_URL . "=$id"),
					// FIXME:
					//'U_SEARCHPOSTS' => append_sid("search.$phpEx?search_author=" . (($id == ANONYMOUS) ? 'Anonymous' : urlencode($username)) . "&amp;showresults=topics"))
					'U_SEARCHPOSTS' => append_sid(basefile . "page=search&search_author=" . urlencode($username) . "&amp;showresults=topics"))
				);
	...
	default:
		...
		make_jumpbox(basefile . 'page=modcp');
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			$u_view_topic = basefile . "page=modcp&mode=split&amp;" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
		...
		$template->assign_vars(array(
			...
			'PAGINATION' => generate_pagination(basefile . "page=modcp&" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'], $forum_topics, $board_config['topics_per_page'], $start),
