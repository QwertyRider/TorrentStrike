...
if ($_GET['h'])
{
	begin_frame("Admin User Search Instructions",false,5);
	begin_table(false);
	echo "<tr><td class=\"row2\"><div align=\"center\">\n
	Fields left blank will be ignored;<br/>\n
	Wildcards * and ? may be used in Name, Email and Comments, as well as multiple values\n
	separated by spaces<br/> (e.g. 'wyz Max*' in Name will list both users named\n
...
$highlight = " class=\"row2\"";

?>

<form method="get" action="<?=$_SERVER["PHP_SELF"]?>">
<?begin_frame("Admin User Search",false,5);
begin_table(true);
?>
<tr>
  <td valign="middle" class="rowhead">Name:</td>
  <td <?=$_GET['n']?$highlight:"class=\"row1\" "?>><input name="n" type="text" value="<?=$_GET['n']?>" size="35"/></td>

  <td valign="middle" class="rowhead">Ratio:</td>
  <td<?=$_GET['r']?$highlight:" class=\"row1\""?>><select name="rt">
    <?
  ...
  <td<?=$_GET['st']?$highlight:" class=\"row1\""?>><select name="st">
    <?
...
<tr><td valign="middle" class="rowhead">Email:</td>
  <td<?=$_GET['em']?$highlight:" class=\"row1\""?>><input name="em" type="text" value="<?=$_GET['em']?>" size="35"/></td>
  <td valign="middle" class="rowhead">IP:</td>
  <td<?=$_GET['ip']?$highlight:" class=\"row1\""?>><input name="ip" type="text" value="<?=$_GET['ip']?>" maxlength="17"/></td>

  <td valign="middle" class="rowhead">Account status:</td>
  <td<?=$_GET['as']?$highlight:" class=\"row1\""?>><select name="as">
    <?
...
<tr>
  ...
  <td<?=$_GET['co']?$highlight:" class=\"row1\""?>><input name="co" type="text" value="<?=$_GET['co']?>" size="35"/></td>
  <td valign="middle" class="rowhead">Mask:</td>
  <td<?=$_GET['ma']?$highlight:" class=\"row1\""?>><input name="ma" type="text" value="<?=$_GET['ma']?>" maxlength="17"/></td>
  <td valign="middle" class="rowhead">Class:</td>
  <td<?=($_GET['c'] && $_GET['c'] != 1)?$highlight:" class=\"row1\""?>><select name="c"><option value='1'>(any)</option>
  <?
  $class = $_GET['c'];
  if (!is_valid_id($class))
  	$class = '';
  $maxclass = get_user_class();
  for ($i = 2;$i< $maxclass+2;$i++) {
...
<tr>
  ...
  <td<?=$_GET['d']?$highlight:" class=\"row1\""?>><select name="dt">
    <?
  ...
  <td<?=$_GET['ul']?$highlight:" class=\"row1\""?>><select name="ult" id="ult">
    <?
    $options = array("equal","above","below","between");
    for ($i = 0; $i < count($options); $i++){
  ...
  <td<?=$_GET['do']?$highlight:" class=\"row1\""?>><select name="do">
    <?
...
<td valign="middle" class="rowhead">Last seen:</td>
  ...
  <td <?=$_GET['ls']?$highlight:" class=\"row1\""?>><select name="lst">
  <?
  $options = array("on","before","after","between");
  for ($i = 0; $i < count($options); $i++){
  ...
  <td<?=$_GET['dl']?$highlight:" class=\"row1\""?>><select name="dlt" id="dlt">
  <?
	$options = array("equal","above","below","between");
	for ($i = 0; $i < count($options); $i++){
	...
	<td<?=$_GET['w']?$highlight:" class=\"row1\""?>><select name="w">
  <?
  $options = array("(any)","Yes","No");
	for ($i = 0; $i < count($options); $i++){
...
<tr><td class="rowhead"></td><td class="row1"></td>
  <td valign="middle" class="rowhead">Active only:</td>
	<td<?=$_GET['ac']?$highlight:" class=\"row1\""?>><input name="ac" type="checkbox" value="1" <?=($_GET['ac'])?"checked":"" ?>/></td>
  <td valign="middle" class="rowhead">Disabled IP: </td>
  <td<?=$_GET['dip']?$highlight:" class=\"row1\""?>><input name="dip" type="checkbox" value="1" <?=($_GET['dip'])?"checked":"" ?>/></td>
  </tr>
<tr><td colspan="6" class="row1" align="center"><input name="submit" type="submit" class="btn"/></td></tr>
<? end_table();
end_frame();?>
<br/><br/>
</form>
  ...
  else
  {
    ...
    while ($user = mysql_fetch_array($res))
    {
      ...
      $n_comments = $n[0];
    			...
    			echo "<tr><td class='row2'><b><a href='userdetails.php?id=" . $user['id'] . "'>" .
...
//					($user["warned"] == "yes" ? "<img src=\"/pic/warned.gif\" alt=\"Warned\">" : "") . "</td>
          "<td class='row2'>" . ratios($user['uploaded'], $user['downloaded']) . "</td>
          <td class='row2'>" . $ipstr . "</td>
          <td class='row2'>" . $user['email'] . "</td>
          <td class='row2'><div align=\"center\">" . $user['added'] . "</div></td>
          <td class='row2'><div align=\"center\">" . $user['last_access'] . "</div></td>
          <td class='row2'><div align=\"center\">" . $user['status'] . "</div></td>
          <td class='row2'><div align=\"center\">" . $user['enabled']."</div></td>
          <td class='row2'><div align=\"center\">" . ratios($pul,$pdl) . "</div></td>" .
          "<td class='row2'><div align=\"right\">" . mksize($pul) . "</div></td>
          <td class='row2'><div align=\"right\">" . mksize($pdl) . "</div></td>
          <td class='row2'><div align=\"center\">".($n_posts?"<a href=\"userhistory.php?action=viewposts&amp;id=".$user['id']."\">$n_posts</a>":$n_posts).
