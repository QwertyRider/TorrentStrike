...
if ($user["donor"] == "yes") $donor = "<img src=\"pic/starbig.gif\" alt=\"Donor\" style='margin-left: 4pt'/>";
if ($user["warned"] == "yes") $warned = "<img src=\"pic/warnedbig.gif\" alt=\"Warned\" style='margin-left: 4pt'/>";

//print("<table class=\"main\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">".
print ("<h1 align=\"center\" style='margin:0px'> Personal lists for $user[username]</h1>$donor$warned$country\n");

//print("<table class=\"main\" width=\"750\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\">");

begin_frame("Friends list");
...
else
{
	...
	while ($block = mysql_fetch_array($res))
	{
		if ($i % 6 == 0)
			$blocks .= "<tr><td>";
    	$blocks .= "<font class=\"small\"><a href=\"friends.php?id=$userid&amp;action=delete&amp;type=block&amp;targetid=" .
				$block['id'] . "\">D</a></font> <a href=\"userdetails.php?id=" . $block['id'] . "\"><b>" . $block['name'] . "</b></a>" .
				get_user_icons($block) . "</td></tr>";
		if ($i % 6 == 5)
			$blocks .= "</td></tr>";
		$i++;
	}
...
print("<br/>");
begin_frame("Blocked users list");
print("$blocks\n");
end_frame();

print("<p align=\"center\"><a href=\"users.php\"><b>Find User/Browse User List</b></a></p>");
